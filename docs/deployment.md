...
在 OCI 既有 Hermes 主機更新時，先用 SSH 進主機，再依序確認可能的部署目錄：

```bash
for d in /opt/taiwanlife-monitor /opt/data/twltool /home/ubuntu/.hermes/twltool; do
  if [ -d "$d/.git" ]; then
    echo "$d"
  fi
done
...
若 `git status --short` 有遠端未提交變更，先停止並保留現場，不要覆蓋。
...
巡檢整體上限由 `config/taiwanlife.json` 的 `global_timeout_seconds` 控制，預設 1800 秒。它會在主要階段與 browser check 之間停止後續工作；單一 Playwright 操作仍由 browser timeout 控制。
...
```bash
python3 -B -m unittest discover -s tests -v
python3 -B -m py_compile taiwanlife_monitor/monitor.py tests/test_monitor_core.py
docker compose config
docker compose build taiwanlife-monitor
docker compose run --rm taiwanlife-monitor
```
